...
- 데스크톱 헤더의 마지막 그리드 열은 `max-content`로 두어 동기화 상태와 사용자 칩의 실제 폭을 먼저 확보한다.
- 검색 영역은 기존 360px~760px 범위를 유지하며, 검색 필드 자체에는 `min-width: 0`을 적용해 남은 공간에 맞춰 축소되게 한다.
- `.header-actions`는 `min-width: max-content`를 사용해 검색 조건 버튼 위로 내용이 넘치지 않게 한다.
- 겹침을 숨기는 `z-index`, 잘라내기 또는 JavaScript 너비 계산은 사용하지 않는다.
- 1024px 이하에서는 기존 비PC 검색 패널과 파일 드로어 구조를 그대로 사용한다.
