...
## 15. 후속 수정 계획: 비정렬 네트워크 위치 보존 패킹

### 15.1 문제별 수정 계획

1. 현재 충돌 시 오른쪽으로만 이동하는 패커가 force 결과를 가로 배열처럼 보이게 만들 수 있다. 각 노드의 force 좌표를 기준점으로 유지하고, 충돌할 때 golden-angle 나선형 후보를 순회해 가장 가까운 빈 위치를 선택한다.
2. 후보 위치도 공간 버킷으로 검사해 실제 노드 폭·높이 + 96px 여백을 보장한다. 연결 성분의 상대적 불규칙성은 유지하고, 충돌 해결만 국소적으로 수행한다.

### 15.2 Process Phase와 Gate

- **Phase A — 비정렬 패커**: 나선형 후보 탐색과 버킷 충돌 판정을 구현한다. Gate: 전체 쌍 겹침 0건·최소 96px 간격·force 기준점 대비 국소 이동.
- **Phase B — 시각 회귀**: 네트워크 edge 거리, 캡슐 형태, 라벨 포함 폭, 검색/선택 상태를 회귀 검증한다. Gate: 가로/세로 동일 좌표열이 강제되지 않음.
- **Phase C — 배포**: CI와 Pages smoke check 성공 후 main 병합한다.

### 15.3 실패 시 재수정 Loop

배열화 재현 → 나선 반경·각도·처리 순서 조정 → 축 정렬 지표와 AABB 검증. 겹침 재현 → 버킷 후보 범위와 여백 계산을 조정 → 전체 쌍 회귀를 반복한다.

## 16. 후속 변경 계획: 줌 비례 노드 텍스트

### 16.1 문제별 수정 계획

1. Cytoscape 노드 본체는 줌에 따라 확대되지만 HTML 라벨의 폰트가 고정 px라 확대할수록 상대적으로 작아진다. `cy.zoom()` 값을 제목·키워드 폰트 크기에 곱해 같은 배율로 확대·축소한다.
2. PC와 non-PC의 기본 폰트 크기는 기존 계약을 유지하고, 읽을 수 있는 최소 크기와 레이아웃을 깨지 않는 최대 크기만 clamp한다.

### 16.2 Process Phase와 Gate

- **Phase A — 화면 계산**: 줌 배율 기반 제목/키워드 폰트 계산을 라벨 동기화 함수에 추가한다. Gate: zoom 0.5/1/2에서 폰트 크기가 같은 방향으로 변한다.
- **Phase B — 회귀 검증**: 드래그·pan·검색 dim·캡슐 폭과 라벨 위치를 유지한다. Gate: 라벨이 노드 중심을 따라가고 기존 static UI 계약이 통과한다.
- **Phase C — 배포**: CI와 Pages smoke check 성공 후 main 병합한다.

### 16.3 실패 시 재수정 Loop

텍스트가 고정되거나 반대로 움직임 → zoom 값을 읽는 시점과 inline style 우선순위를 확인 → desktop/non-PC 줌 회귀. 텍스트가 캡슐 밖으로 넘침 → 최대 폰트 clamp 또는 라벨 폭 계산을 조정 → 시각·정적 검증을 반복한다.
