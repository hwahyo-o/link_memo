export function normalizeMemoInput({ text, url, comment, hasImage = false }) {
    ...
    if (!normalizedText) {
        return { ok: false, error: '버튼에 표시될 텍스트를 입력해주세요.' };
    }
    if (rawUrl && !normalizedUrl) {
        return { ok: false, error: '링크는 HTTP 또는 HTTPS 주소만 입력할 수 있습니다.' };
    }
    if (!normalizedUrl && !hasImage && !hasComment) {
        return { ok: false, error: '링크, 이미지 또는 코멘트 중 하나를 입력해주세요.' };
    }
