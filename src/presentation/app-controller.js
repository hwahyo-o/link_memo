Warning: truncated output (original token count: 27973)
...
function migrateDataFormat() {
    ...
    categories.forEach(category => {
        ...
        if (linkData[category].length === 0 || linkData[category][0]?.url !== undefined) {
            ...
            linkData[category].forEach(su…7973 tokens truncated…a-grip-vertical text-gray-300 mr-2 text-sm"></i><i class="fa-solid fa-chevron-down text-gray-400 mr-3 text-sm"></i><h3 class="font-bold text-gray-700 truncate">${escapeHtml(subcategory.title)} <span class="text-xs text-gray-400 ml-1 font-normal">(${subcategory.links.length})</span></h3>`;
