...
async function refreshBackupCatalog() {
    if (!currentUser || currentUser.isAnonymous || backupCatalogLoaded || !backupService.configured()) return;
    try {
        const backups = await backupService.list({ user: currentUser });
        const current = createBackupState(backupState);
        backupState = createBackupState({
            ...current,
            manualBackups: backups.filter(item => item.reason === 'manual'),
            autoBackups: backups.filter(item => item.reason === 'auto')
        });
        backupCatalogLoaded = true;
        renderBackupSettings();
    } catch (error) {
        console.warn('Cloudflare 백업 목록을 불러오지 못했습니다.', error);
    }
}
...
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        ...
        void lifecycleSyncService.flushForPageExit({ keepaliveOnly: false });
    }
});
window.addEventListener('pagehide', () => { void lifecycleSyncService.flushForPageExit({ keepaliveOnly: true }); });
...
function buildMemoPayload() {
    return { categories, linkData, uiPreferences, driveConnection, backupInfo, backupState, syncMeta };
}

function buildBackupPayload() {
    // 백업 성공/실패 기록은 비교 대상에서 제외해 실제 사용자 데이터만 비교합니다.
    return { categories, linkData, uiPreferences, driveConnection, syncMeta };
}
...
window.handleLogout = () => {
    ...
    customConfirm('로그아웃 하시겠습니까?', async () => {
        try {
            await lifecycleSyncService.flushBeforeLogout();
            await signOut(auth);
...
async function restoreLocalMemo(userId) {
    ...
    applyPreferences();
    latestCheckpointPayload = cloneMemoPayload(payload);
    return true;
}
...
function hasPendingDriveImages() {
    return Object.values(linkData || {}).some(subcategories => (subcategories || []).some(subcategory =>
        (subcategory.links || []).some(link => getLinkImages(link).some(image => image?.imageId && !image?.driveImage?.fileId))
    ));
}

async function restoreCloudCheckpoint(userId) {
    if (!currentUser || currentUser.isAnonymous || !backupService.configured()) return false;
    try {
        const checkpoint = await backupService.loadCheckpoint({ user: currentUser });
        if (!checkpoint?.payload || checkpoint.userId !== userId) return false;
        const local = await localMemoRepository.load(userId);
        const merged = mergeMemoPayloads(local?.payload, checkpoint.payload, {
            leftUpdatedAt: local?.updatedAt,
            rightUpdatedAt: checkpoint.updatedAt
        });
        if (!isSameMemoPayload(merged, local?.payload)) {
            const record = await localMemoRepository.savePending(userId, merged, { remoteRevision: local?.remoteRevision || 0 });
            applyMemoPayload(record.payload);
            localMemoDirty = true;
            latestCheckpointPayload = cloneMemoPayload(record.payload);
            dataLoadState = 'ready';
            return true;
        }
    } catch (error) {
        if (error?.message !== 'CHECKPOINT_NOT_FOUND') console.warn('종료 체크포인트 복구 실패', error);
    }
    return false;
}
...
function loadDataFromFirestore() {
    ...
    unsubscribeSnapshot = memoRepository.subscribe(currentUser.uid, async snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            const remotePayload = data.syncMeta
                ? data
                : prepareLocalMemoPayload(null, data, { now: Number(data.updatedAt || Date.now()), deviceId: 'legacy-remote' });
            const merged = mergeMemoPayloads(local?.payload, remotePayload, {
                leftUpdatedAt: local?.updatedAt,
                rightUpdatedAt: data.updatedAt
            });
            applyMemoPayload(merged);
            latestCheckpointPayload = cloneMemoPayload(merged);
            const mustUploadMerge = Boolean(local?.dirty) || !data.syncMeta || !isSameMemoPayload(merged, remotePayload);
            if (mustUploadMerge) {
                await localMemoRepository.savePending(currentUser.uid, merged, { remoteRevision: memoRevision });
                localMemoDirty = true;
                scheduleMemoSync();
            } else {
                localMemoDirty = false;
                await localMemoRepository.cache(currentUser.uid, merged, { remoteRevision: memoRevision });
            }
            ...
            const local = await localMemoRepository.load(currentUser.uid);
            if (local?.payload) {
                applyMemoPayload(local.payload);
                memoRevision = 0;
                memoSyncService.setRevision(0);
                localMemoDirty = true;
                dataLoadState = 'ready';
                scheduleMemoSync({ allowCreate: true });
            } else {
                categories = [...DEFAULT_CATEGORIES];
...
async function createCloudBackup(reason, scheduledFor = null) {
    ...
    // 자동 백업은 직전 자동 백업과 동일한 페이로드면 R2 쓰기를 생략합니다.
    const latestBackup = reason === 'auto' ? (backupState.autoBackups?.[0] || null) : (getBackupList(backupState)[0] || null);
...
async function writeLocalMemo() {
    ...
    const previous = await localMemoRepository.load(userId);
    const payload = prepareLocalMemoPayload(previous?.payload, cloneMemoPayload(buildMemoPayload()), {
        now: Date.now(),
...
window.saveLink = async () => {
    ...
    if (files.length) {
        const task = processSelectedImagesInBackground(link.id, files);
        pendingImageTasks.add(task);
        void task.finally(() => pendingImageTasks.delete(task));
    }
};
